...
# Regressions: Predict fixed mutation on gyrA based on number of effective drugs  # 
#---------------------------------------------------------------------------------#
...
# 2. regress number of effective drugs on mutation on gyrA----
# simplest model with only number of effective drugs
model_simple <- glm(Rv0006 ~ tx_effective_drugs, data = data, family = binomial(link = 'logit'))
...
results_simple <- rbind(results_simple, row1, row2)
...
write.csv(results_simple, 'output/treatment_hetero_mutation/cont_neffectdrug_Rv0006_reg_results.csv')

# adding in main lineage
model_lineage <- glm(Rv0006 ~ tx_effective_drugs + main_lineage2 + main_lineage4, data = data, family = binomial(link = 'logit'))
...
# extract coefficients on main_lineage
cat3 <- rownames(coef(summary(model_lineage)))[3] # lineage2
...
cat4 <- rownames(coef(summary(model_lineage)))[4] # lineage4
...
results_lineage <- rbind(results_lineage, row1, row2, row3, row4)
...
write.csv(results_lineage, 'output/treatment_hetero_mutation/cont_neffectdrug_Rv0006_lineage_reg_results.csv')

# adding in baseline resistance 
model_baseresist <- glm(Rv0006 ~ tx_effective_drugs + baseline_resistance_type, data = data, family = binomial(link = 'logit'))
...
cat3 <- rownames(coef(summary(model_baseresist)))[3] # mono-resistant
...
cat4 <- rownames(coef(summary(model_baseresist)))[4] # resistant
...
cat5 <- rownames(coef(summary(model_baseresist)))[5] # mdr
...
cat6 <- rownames(coef(summary(model_baseresist)))[6] # xdr
...
results_baseresist <- rbind(results_baseresist, row1, row2, row3, row4, row5, row6)
...
write.csv(results_baseresist, 'output/treatment_hetero_mutation/cont_neffectdrug_Rv0006_baseresist_reg_results.csv')

# full model with both main lineage and baseline resistance
model_full <- glm(Rv0006 ~ tx_effective_drugs + main_lineage2 + main_lineage4 + baseline_resistance_type, data = data, family = binomial(link = 'logit'))
summary(model_full)
...
# extract coefficients on main_lineage
cat3 <- rownames(coef(summary(model_full)))[3] # lineage2
...
cat4 <- rownames(coef(summary(model_full)))[4] # lineage4
...
# extract coefficients on baseline resistance 
cat5 <- rownames(coef(summary(model_full)))[5] # mono-resistant
...
cat6 <- rownames(coef(summary(model_full)))[6] # resistant
...
cat7 <- rownames(coef(summary(model_full)))[7] # mdr
...
cat8 <- rownames(coef(summary(model_full)))[8] # xdr
...
results_full <- rbind(results_full, row1, row2, row3, row4, row5, row6, row7, row8)
...
write.csv(results_full, 'output/treatment_hetero_mutation/cont_neffectdrug_Rv0006_allcovars_reg_results.csv')

...
# gene <- read.table('data/mtb_wh/gene_table_by_strainID.txt', sep = '\t', header = T)
gene <- readRDS('datasets/fixed_mutations_nmj.Rds')
...
treatment_mutation <- treatment_clean %>% 
  merge(gene, by = 'strain_id') %>% 
...
treatment_mutation <- treatment_mutation %>% 
...
# 8. save dataset----
saveRDS(treatment_mutation, file = "~/Desktop/LSHTM/Summer Project/analysis/datasets/tx_fixedmut.Rds")
